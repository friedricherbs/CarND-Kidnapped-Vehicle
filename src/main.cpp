...
int main() {
	...
	// Run particle filter!
	const unsigned int num_time_steps = position_meas.size();
	ParticleFilter pf;
	...
	for (unsigned int i = 0; i < num_time_steps; ++i) {
		...
		const double * const avg_error = getError(gt[i].x, gt[i].y, gt[i].theta, best_particle.x, best_particle.y, best_particle.theta);
	...
	const int stop = clock();
	const double runtime = (stop - start) / double(CLOCKS_PER_SEC);
